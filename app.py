...
# ============================================================
# 0. قاعدة البيانات - اتصال وتهيئة (مدمج من الكود الثاني)
# ============================================================
try:
    import pymysql
...
def get_db_connection():
    """اتصال آمن بقاعدة البيانات يدعم Unix Socket و TCP/IP."""
    if not MYSQL_AVAILABLE:
        st.error("❌ مكتبة pymysql غير مثبتة. قم بتشغيل: pip install pymysql")
        return None
    try:
...
# ============================================================
# 1. دوال قاعدة البيانات المحلية (بديل عن cloudsql_utils)
# ============================================================
...
def save_project_plan(plan_json: dict, user_id: int) -> bool:
    ...
    try:
        with conn.cursor() as cursor:
            sql = """
                INSERT INTO projects (client_name, summary, budget_range, tech_stack, user_id)
                VALUES (%s, %s, %s, %s, %s)
            """
            cursor.execute(sql, (
...
# ============================================================
# 2. نظام AI Gateway (إدارة مفاتيح Gemini المرنة)
# ============================================================
...
# ============================================================
# 3. نظام المصادقة (تسجيل الدخول / إنشاء حساب) - باستخدام get_db_connection
# ============================================================
def init_auth():
...
# ============================================================
# 4. نظام الفريميوم (5 استخدامات مجانية)
# ============================================================
def init_usage():
...
# ============================================================
# 5. دوال الدفع (Lemon Squeezy) - باستخدام os.getenv بدلاً من config
# ============================================================
...
# ============================================================
# 8. توليد الخطة (RAG + Gemini)
# ============================================================
def generate_project_plan_safe(api_key: str, interview_data: dict) -> dict:
    genai.configure(api_key=api_key)
    model = genai.GenerativeModel("gemini-2.5-flash")
    similar_plans = get_similar_projects(interview_data["idea"], top_k=2)
    similar_context = ""
    if similar_plans:
...
# ============================================================
# 10. محرك التحليل المتقدم (Advanced Analytics Engine)
# ============================================================
...
def render_advanced_analytics(plan_json: dict):
    ...
    with col_chart1:
        ...
        st.plotly_chart(fig1, use_container_width=True, key="analytics_priority_pie")
    with col_chart2:
        if tasks:
            ...
            st.plotly_chart(fig2, use_container_width=True, key="analytics_days_bar")
    ...
    with col_chart3:
        if tasks:
            ...
            st.plotly_chart(fig3, use_container_width=True, key="analytics_cost_pie")
    with col_chart4:
        ...
        st.plotly_chart(fig4, use_container_width=True, key="analytics_gauges")
...
# ============================================================
# 11. لوحة التحكم المتطورة (مع التحليلات)
# ============================================================
